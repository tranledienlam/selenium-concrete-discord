...
📌 **Trang dự án**: [concrete.xyz](https://points.concrete.xyz/home)
...
<p align="center">
    <a href="https://youtu.be/-Vp8AnG22ac">
        <img src="https://img.youtube.com/vi/-Vp8AnG22ac/maxresdefault.jpg" alt="Xem video demo">
    </a>
</p>
...
- **Đăng nhập Discord**: Khi cung cấp email/mật khẩu Discord, tool sẽ tự động đăng nhập tài khảon Discord. (Nếu gặp captcha, tool sẽ không thể đăng nhập. Cần tự đăng nhập thủ công)
- **Gửi tin nhắn**: AI Gemini sẽ đọc tin nhắn trên màn hình và gửi tin nhắn đến kênh. Nếu AI không hoạt động, thì tool sẽ lấy random tin nhắn tại MESSAGES (1 trong 20 tin) trong `index.py`

---

## 🔧 Yêu cầu ban đầu

- **Discord**: Đăng nhập trước Discord (nếu không cung cấp email/mật khấu) và yêu cầu đã tham gia channel (https://discord.gg/concretexyz)
- **Token AI Gemini**: Cần lấy token AI Gemini [Hướng dẫn](#-tips)
---
...
| File                             | Mô tả                                      |
| -------------------------------- | ------------------------------------------ |
| `discord.py`                       | Xử lý các thao tác liên quan đến Discord.   |
...
- Mỗi dòng chứa thông tin một profile theo cấu trúc:
  ```plaintext
  <tên_profile>|<email (tuỳ chọn 1)>|<mật_khẩu_discord (tùy chọn 2)>|<proxy (tuỳ chọn)>
  ```

  Trong đó:

    - `tên_profile`: Tên của profile.
    - `email (tuỳ chọn 1)`: địa chỉ gmail.
    - `mật_khẩu_discord (tùy chọn 2)`: mật khẩu đăng nhập gmail. (Nếu không cung cấp, hãy thực hiện đăng nhập thủ công)
...
#### 🤖 **Cấu hình AI Bot (Tool này yêu cầu)**
